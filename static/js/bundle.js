...
$(function(){
  ...
  function initializeControls() {
	var clipboard = new Clipboard('.copyhelpurl');	  
	clipboard.on('success', function(e) {
		linkStatus("link.copied", 3000);
	});
	...
	//$("#copy").click(function(){
	//	 window.prompt("请复制(Ctrl+C)", $(".copyable").val());
	//});
  ...
  function googleItForThem() {
    ...
    function type(string, index){
      ...
      if (index < string.length) {
        setTimeout(function(){ type(string, index + 1); }, Math.random() * 700);
      }
      else {
        doneTyping();
      }
